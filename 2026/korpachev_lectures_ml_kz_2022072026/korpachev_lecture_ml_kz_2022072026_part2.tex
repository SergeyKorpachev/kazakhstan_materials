...
\centering
\includegraphics[width=0.3\textwidth]{data_and_fit.png}
...
\begin{itemize}
\footnotesize
\item[\mybullet] What about stopping criteria?
\begin{enumerate}
	\vspace{1ex}
	\itemsep0.8ex
	\footnotesize
\item Split until one element in leaf \\ \textcolor{gray}{(which problems can we meet?)}
\item Limit max depth/max number of leaves
\item Limit amount of elements in leaf/split
%\item Limit amount of elements to split
\item All elements in a leaf belong to a single class\\
\item $\text{IG}(j, t)$ isn't improving by x\%
\item Combination of limitations
\end{enumerate}

\item<2>[\textcolor{myblack}{\MVRightArrow}] The choice is up to you \\(aka hyperparameter)

%\begin{textblock*}{0.5\textwidth}(0.05\paperwidth, 0.6\paperheight)
%%\centering
%\onslide<2->{The choice is up to you (aka hyperparameter)}
%\end{textblock*}
...
\begin{figure}
	\centering
	\includegraphics[width=1.1\textwidth]{4tree.png}
\end{figure}

\end{columns}
%\begin{figure}
%	\centering
%	\includegraphics<2->[width=0.7\textwidth]{drift.jpg}
%\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{bagging_subspace_sampling2.png}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width = 0.7\textwidth]{bagging.png}
\label{fig:my_label}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width = 0.35\textwidth]{rf.png}
\end{figure}
%https://towardsdatascience.com/decision-trees-and-random-forests-for-classification-and-regression-pt-2-2b1fcd03e342
\end{frame}
...
\begin{textblock*}{.9\paperwidth}(0.05\paperwidth, 0.25\paperheight)
\vspace{0.3cm}
\begin{itemize}
	\itemsep.5ex
	\small
	\setbeamertemplate{items}{\mybullet}
	\item Ensembling models improves performance comparing to a single model
	\item However, simply training them independently sounds not quite optimal (although is easy to parallelize)
	\item Maybe, we can add models to ensemble in a cleverer way?
	\only<2->{\item[\textcolor{myblack}{\MVRightArrow}] e.g., iteratively by correcting upon mistakes of previous models?}
\end{itemize}

\only<2->{
	\footnotesize
	\centering
	$a_{n}(x)=h_{1}(x)+\cdots+h_{n}(x)$
	\begin{figure}
		\centering
		\includegraphics[width = 0.5\textwidth]{visboosting.png}
	\end{figure}
}
\end{textblock*}
...
\myframetitle{0.4\paperwidth}{0.04\paperwidth}{Section summary}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\small
			\item binary decisions to model non-linearities
			\item grow greedily leaf by leaf
			\item split leaves to improve Information Gain
			\item until stopping criteria is reached
			\item predict with constant in a leaf after propagating through
		\end{itemize}
		\item \textbf{Random forest}
		\begin{itemize}
			\small
			\setbeamertemplate{items}{\mysubbullet}
			\item combine trees into ensemble for smaller error
			\item each tree is independent of others
			\item low-biased trees + RSM + bagging
		\end{itemize}
		\item \textbf{Gradient boosting}
		\begin{itemize}
			\setbeamertemplate{items}{\mysubbullet}
			\item combine algorithms into ensemble for even smaller error
			\item each next algorithm improves predictions of previous ones
		\end{itemize}
	\end{itemize}
