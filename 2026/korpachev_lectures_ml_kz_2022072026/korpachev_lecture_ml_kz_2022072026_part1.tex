...
\only<2->{\includegraphics[width=0.27\textwidth]{ai_ml_dl.png}}
\hspace{0.07\textheight}
\only<3->{\includegraphics[width=0.25\textwidth]{approximation.png}}
...
\begin{itemize}
	\item[\mybullet] Information about goods and sales
	\item[\mybullet] Data from subsystems of detector
	\item[\mybullet] Subscribers database
\end{itemize}
Shall one question the Data:
\begin{itemize}
	\item[\mybullet] predict demand for this product?
	\item[\mybullet] estimate energy of a particle?
	\item[\mybullet] like or dislike the post? 
\end{itemize}
\vspace{0.05\paperheight}
How can we answer these questions? \uncover<2>{\textcolor{myorange}{\MVRightarrow \hspace{0.4mm} this lecture}}

\column{0.35\textwidth}
\hspace{-0.07\paperwidth}
\vspace{0.17\paperheight}
\includegraphics[width=0.35\paperwidth]{pileup.png} %what-is-data-science.jpg	
...
\subsection{Can we use data out of the box?}
\begin{frame}
...
\myframesubtitle{0.37\paperwidth}{\insertsubsection}
	...
	\begin{columns}
		...
		\begin{itemize}
			\itemsep0.8ex
			\item[\mybullet] Most likely, no: garbage in, garbage out!
			\item[\mybullet] Main drawbacks of raw data:
			\begin{itemize}
				\itemsep0.8ex
				\item[\mysubbullet] Not in ML comprehensive format
				\item[\mysubbullet] Can have missing values or might be noisy
				\item[\mysubbullet] Useless/lack of useful features 
			\end{itemize}
			\item[\textcolor{myorange}{\textbf{!}}] Analysing data improves its understanding 
		\end{itemize}%
		\column{0.4\textwidth}
		\vspace{0.33\paperheight}
		\begin{minipage}[c][.66\textheight][t]{\linewidth}
			\centering
			\includegraphics[width=0.9\linewidth]{garbage_tracks.png}\\
		\end{minipage}
	\end{columns}
\transfade[duration=.4]
\end{frame}
...
\subsection{Comprehensive format}
\begin{frame}
...
%	\hspace{2cm}
	...
	\begin{itemize}
		\item[\mybullet] categorical data require one hot encoding (or any different method)
		\item[\mybullet] text should also be vectorized 
		\item[\mybullet] pictures can be treated similarly
	\end{itemize}
	\column{0.55\textwidth}
	\vspace{0.25\paperheight}
	\begin{minipage}[c][.99\textheight][t]{\linewidth}
		\center
		\includegraphics[width=0.8\linewidth]{categorical.png}\\ \vspace{0.5em}
		\includegraphics[width=0.6\linewidth]{text2features.png}
		\includegraphics[width=0.6\linewidth]{picture2features.png}
	\end{minipage}
\end{columns}

\begin{textblock*}{0.4\paperwidth}(0.7\paperwidth,0.1\paperheight)
	\small
	\textcolor{mypurple}{more in the seminar!}
\end{textblock*}
...
\begin{columns}
	...
	Main techniques:
		\begin{itemize}
			\item[\mybullet] Imputation of missing data
			\item[\mybullet] Data cleaning
			\item[\mybullet] Handling outliers
			\item[\mybullet] Shuffling
			\item[\mybullet] Balancing classes
			\item[\mybullet] Partitioning
		\end{itemize}%

	\column{0.6\textwidth}
	\begin{figure}[t]
		\centering
		\includegraphics[width=.9\linewidth]{data_preprocessing.png}
		\includegraphics[width=.8\linewidth]{corrupted_data.png}				
	\end{figure}
...
%			Feature engineering techniques:
			\begin{itemize}
				\item[\mybullet] \textbf{Construction:} vectorization techniques + your domain knowledge
				\item[\mybullet] \textbf{Transformation:} functions, scaling, binning, croping, resizing, blurring
				\item[\mybullet] \textbf{Selection:} PCA, statistical tests, feature importance, hand-picking
			\end{itemize}
		\end{minipage}%
		\column{0.5\textwidth}
		\begin{minipage}[c][.66\textheight][t]{\linewidth}
			\flushleft
			\textcolor{myorange}{Example:} Using using Polar coordinates it's much easier to separate \textcolor{Red}{red} dots from \textcolor{Blue}{blue} than using Cartesian coordinates:
			\begin{figure}[t]
				\centering
				\includegraphics[width=0.8\linewidth]{feature_engineering.png}		%\subfigure{\includegraphics[width=0.5\linewidth]{Outlier}}
				%\subfigure{\includegraphics[width=0.4\linewidth]{Grouping}}
			\end{figure}
		\end{minipage}
	\end{columns}
...
\begin{minipage}[c][.99\textheight][t]{\linewidth}
		...
		\begin{figure}[t]
			...
			\only<2->{\includegraphics[width=0.45\linewidth]{pipeline2.png}}
		\end{figure}	
	\end{minipage}
\transfade[duration=.4]
\end{frame}
...
\subsection{Solution can be found analytically}
\begin{frame}[t]
...
\myframesubtitle{0.37\paperwidth}{\insertsubsection}
	...
	\begin{columns}
		...
		\begin{minipage}[t]{\linewidth}
				\vspace{-1em}
			\flushleft
			After defining feature matrix as:\\[1em]
			$F 	= 
			\begin{bmatrix}
			f_1(\bm{x_{1}}) & \cdots & f_K(\bm{x_{1}})\\
			\vdots     & \ddots & \vdots  \\
			f_1(\bm{x_{N}}) & \cdots & f_K(\bm{x_{N}})\\
			\end{bmatrix} $ \\[1em]
		\end{minipage}%	
		\column{0.5\textwidth}
		\begin{minipage}[c]{\linewidth}
			...
			\begin{itemize}
				\setbeamertemplate{items}{\mybullet}
				\item $Q(\bm{\theta}) = \textcolor{Black}{\cancel{ \frac{1}{N}}} \big(F \bm\theta - \bm y\big)^T \big(F \bm\theta - \bm y\big) = \vspace{2mm}\newline = \bm\theta^T F^T F\bm\theta - 2\bm y^TF\bm \theta + \bm y^T \bm y$ 
				
				\item  $\cfrac{\partial Q}{\partial \bm \theta^T} = 2 F^TF\bm\theta - 2F^T\bm y = 0$
			\end{itemize}	
		\end{minipage}
	\end{columns}
	\centering
	\vspace{0.03\paperheight}
 	\large	$\bm{\hat{\theta}} = (F^TF)^{-1}F^{T}\bm{y}$
	
	\vspace{0.02\paperheight}
	\flushleft
	\normalsize
	 \textcolor{myorange}{Problem 1:} calculation of $(F^TF)^{-1}$ has \textbf{complexity of} $\bm{O(K^5N)} \Rightarrow$ computationally-intensive for large number of features\\
	\textcolor{myorange}{Problem 2:} $F$ is often \textbf{ill-conditioned} $\Rightarrow$ computational errors causes instabilities and extremely large solutions
...
\myframesubtitle{0.37\paperwidth}{\insertsubsection}
	...
	\begin{columns}
		...
		\begin{minipage}[c][.66\textheight][t]{\linewidth}
			\begin{itemize}
				\setbeamertemplate{items}{\mybullet}
				\item Let's consider a problem where objects have only one feature: $\bm{x} = [1, f_1(\bm{x})]$
				\item Then  $\bm{\theta} = [\theta_0,\theta_1]$
				\item How can we find minimum of $Q(\bm \theta)$ without solving matrix equation?
			\end{itemize}%
		...
		\column{0.5\textwidth}
			\begin{figure}[c]
				\centering
				\vspace{-0.3\paperheight}
				\includegraphics[width=1\linewidth]{data_and_fit.png}
			\end{figure}
	\end{columns}
...
%				\setbeamertemplate{items}{\color{mypurple}{\rotatebox[origin=c]{-90}{\MVRightarrow}}}
				\setbeamertemplate{items}{\mybullet}
				\item Start with some vector $\bm{\theta^0}$
				\item Initialize $\hat{\bm{\theta}} = \bm{\theta^0}$
				\setbeamertemplate{items}{\textcolor{myblack}{\rotatebox[origin=c]{-90}\MVRightarrow}}
				\item Take randomly another vector $\bm{\theta^\prime}$
				\item Compare $Q(\bm{\theta^\prime})$ and $Q(\hat{\bm{\theta}})$
				\item $\hat{\bm{\theta}} = \text{arg}~ \underset{\hat{\bm{\theta}}, \bm{\theta^\prime}}{\text{min}}~(Q(\hat{\bm{\theta}}),Q(\bm{\theta^\prime}))$
				\item Repeat until convergence
			\end{itemize}
		\vspace{3mm}
		\textcolor{myorange}{Problem:} random search of $\bm{\hat \theta}$ is not effective for practical use in high dimensional $\Theta$ space
		\end{minipage}%
		\column{0.4\textwidth}
		\begin{minipage}[c][.66\textheight][t]{\linewidth}
				\begin{figure}[c]
					\centering
					\vspace{-0.05\paperheight}
					\includegraphics[width=1\linewidth]{grad_d_rand.png}
					\small
					Q as a function of $\theta_0$ and $\theta_1$. \\Blue dots represent Q values for different $\bm{\theta}$
				\end{figure}
		\end{minipage}
	\end{columns}
...
\myframesubtitle{0.37\paperwidth}{\insertsubsection}
	...
	\begin{columns}
		\column{0.5\textwidth}
		\begin{minipage}[c][.66\textheight][t]{\linewidth}
			\textcolor{mygreen}{Idea:} bring up additional information about the direction to the minimum of Q using its \textbf{antigradient}
			\begin{itemize}
				\setbeamertemplate{items}{\mybullet}
%				\setbeamertemplate{items}{\color{mypurple}{\rotatebox[origin=c]{-90}{\MVRightarrow}}}
				\item Start with some vector $\bm{\theta^0}$
				\item $\hat{\bm{\theta}} \leftarrow \bm{\theta^0}$
				\setbeamertemplate{items}{\color{myblack}{\rotatebox[origin=c]{-90}\MVRightarrow}}
				\item Calculate direction to the minimum: $-\nabla Q(\hat{\bm{\theta}}) = -\biggl[\cfrac{\partial Q}{\partial \theta_0},\cfrac{\partial Q}{\partial \theta_1} \biggr]_{\hat{\bm{\theta}}}$
				\item Move towards the minimum: $\hat{\bm{\theta}} \leftarrow \hat{\bm{\theta}} - \textcolor{myorange}{\eta} \nabla Q(\hat{\bm{\theta}})$
%				\setbeamertemplate{items}{\textcolor{mypurple}{$\bullet$}}
				\item Repeat until convergence
			\end{itemize}%
		\end{minipage}%
		\column{0.5\textwidth}
		...
		\begin{minipage}[c][.66\textheight][t]{\linewidth}
			\begin{figure}[t]
				\centering
				\includegraphics[width=0.7\linewidth]{grad_d.png}		
			\end{figure}
%		\vspace{0.01\paperheight}
		\begin{figure}[t]
			\centering
			\includegraphics[width=0.7\linewidth]{data_and_models.png}		
		\end{figure}
		\end{minipage}
	\end{columns}
...
\myframetitle{0.4\paperwidth}{0.04\paperwidth}{Section summary}
%\vspace{0.3\paperheight}

\begin{textblock*}{0.95\paperwidth}(0.05\paperwidth, 0.22\paperheight)
\begin{itemize}
	\footnotesize
	\itemsep.8ex
	\item[\mybullet] All ML problems start from:
	\begin{itemize}
		\footnotesize
		\item[\mysubbullet] Data preprocessing
		\item[\mysubbullet] Feature engineering
	\end{itemize}
	\item[\mybullet] Type of model/approach is defined by our intentions:
	\begin{itemize}
		\footnotesize
		\item[\mysubbullet] \textcolor{myorange}{\textbf{SL:}} seek for a \textbf{predictive model}: for a given object based on its \textbf{features} returns the \textbf{answer} 
		\begin{itemize}
			\scriptsize
			\item[--] classification
			\item[--] regression
			\item[--] ranking
		\end{itemize}
		\item[\mysubbullet] \textcolor{mypurple}{\textbf{UL:}} look for insights into the \textbf{structure of data}
		\item[\mysubbullet] \textcolor{mygreen}{\textbf{RL:}} want to build an agent capable of \textbf{acting in the given environment}
	\end{itemize}
	\item[\mybullet] Model is constructed through the \textbf{training process} involving \textbf{loss function optimization}
	\item[\mybullet] Once we are sure that the model is reasonable we can \textbf{deploy it in production}
	\item[\mybullet] \textbf{Linear models} are the simplest example of models used for \textcolor{myorange}{SL}
	\item[\mybullet] Solution: analytically, but \textbf{(stochastic) gradient descent} is a more general approach 
\end{itemize}
\end{textblock*}
